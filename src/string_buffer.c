...
inline void string_buffer_append_char(String_Buffer* string_buffer, char c) {
	...
	*(string_buffer->data + string_buffer->count) = '\0';
}
...
// resetting the count and setting the null byte as the first character.
...
inline void string_buffer_clear(String_Buffer* string_buffer) {
	memset(string_buffer->data, 0, string_buffer->count * STRING_BUFFER_SIZEOF);
	string_buffer->count = 0;
}
